...
public class ClawMovement : MonoBehaviour
{
    ...
    void ignoreCollisions(bool isOn)
    {
        ...
        GameObject[] random;
        random = GameObject.FindGameObjectsWithTag("IgnorePls");

        for (int i = 0; i < random.Length; i++)
        {
            Physics2D.IgnoreCollision(random[i].GetComponent<CircleCollider2D>(), leftO.GetComponent<EdgeCollider2D>(), isOn);
            Physics2D.IgnoreCollision(random[i].GetComponent<CircleCollider2D>(), rightO.GetComponent<EdgeCollider2D>(), isOn);
        }
